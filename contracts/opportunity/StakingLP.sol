    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public constant REWARD_RATE_MULTIPLIER = 1e6;
    ...
    function initialize(
        ...
        require(
            _rewardsDuration > 0,
            "LPStaking: rewards duration is not valid"
        );
    ...
    modifier updateReward(address _account) {
        ...
        lastUpdatedTime = startTime >= lastTimeRewardApplicable()
            ? startTime
            : lastTimeRewardApplicable();
        if (_account != address(0)) {
            lpStakeholders[_account].lpRewards = earned(_account);
            lpStakeholders[_account]
    ...
    function stakeLP(
        ...
        require(
            opportunityContract != address(0) && resonateAdapter != address(0),
            "LPStaking: address is not valid"
        );
        require(
            msg.sender == opportunityContract || msg.sender == resonateAdapter,
            "LPStaking: caller is not verified"
        );
        require(_amount > 0, "LPStaking: cannot stake 0");
        if (startTime > block.timestamp) {
            require(
                eligibleUsers[_account],
        ...
        returns (uint256)
    {
        ...
        require(
            lpStakeholders[msg.sender].exist,
            "LPStaking: lpStakeholder does not exist"
        );

        uint256 currentLPRewards = lpStakeholders[msg.sender].lpRewards;
        require(
            currentLPRewards >= _lpRewards,
            "LPStaking: not enough balance"
        );
    ...
    function withdraw(
        ...
        external
        nonReentrant
        updateReward(_originAccount)
        returns (uint256, uint256)
    {
        address _resonateAdapter = resonateAdapter; // gas saving
        require(
            opportunityContract != address(0) && _resonateAdapter != address(0),
            "LPStaking: address is not valid"
        );
        require(
            msg.sender == opportunityContract || msg.sender == _resonateAdapter,
            "LPStaking: caller is not verified"
        );
        if (msg.sender == _resonateAdapter) {
            require(
                _originAccount == _resonateAdapter,
                "LPStaking: _originAccount is not valid"
            );
        }
        require(_lpAmount > 0, "LPStaking: cannot withdraw 0");
        require(
            lpStakeholders[_originAccount].exist,
            "LPStaking: lpStakeholder does not exist"
        );
    ...
    function notifyRewardAmount(
        ...
        if (block.timestamp >= periodFinish) {
            rewardRate = (reward * REWARD_RATE_MULTIPLIER) / rewardsDuration;
            _rewards = reward;
        } else {
            uint128 currentTime = startTime >= block.timestamp
                ? startTime
            ...
            rewardRate =
                ((reward + leftover) * REWARD_RATE_MULTIPLIER) /
                rewardsDuration;
        ...
        require(
            rewardRate <= (balance * REWARD_RATE_MULTIPLIER) / rewardsDuration,
            "LPStaking: Provided reward too high"
        );
        lastUpdatedTime = startTime >= block.timestamp
            ? startTime
            : uint128(block.timestamp);
        periodFinish = lastUpdatedTime + rewardsDuration;
    ...
    function setRewardsDuration(uint128 _rewardsDuration) external onlyOwner {
        if (addresses.length > 0 && block.timestamp < periodFinish) {
            for (uint256 i = 0; i < addresses.length; i++) {
                ...
                lpStakeholders[stakeHolderAddress].lpRewards = earned(
                    stakeHolderAddress
                );
                lpStakeholders[stakeHolderAddress]
                    .userRewardPerTokenPaid = rewardPerTokenStored;
            }
        }
        ...
        if (_startTime >= block.timestamp) {
            ...
            rewardRate = (_rewards * REWARD_RATE_MULTIPLIER) / _rewardsDuration;
        } else {
            require(
                _startTime + _rewardsDuration > block.timestamp,
            ...
            rewardRate =
                (oldRemainingTime * rewardRate) /
                (_rewardsDuration + _startTime - block.timestamp);
        }
        periodFinish = _startTime + _rewardsDuration;
        emit RewardsDurationUpdated(oldDuration, _rewardsDuration);
    }

    function setOpportunityContract(
        address _opportunityContract
    ) external onlyOwner {
        require(
            _opportunityContract != address(0),
            "LPStaking: address is not valid"
        );
    ...
    function addToEligibleUsers(
        address[] memory _accountList
    ) external onlyOwner {
        for (uint8 i = 0; i < _accountList.length; i++) {
            eligibleUsers[_accountList[i]] = true;
        }
    }
    ...
    function getRewardForDuration() external view returns (uint256) {
        return (rewardRate * rewardsDuration) / REWARD_RATE_MULTIPLIER;
    }
    ...
    function rewardPerToken() public view returns (uint256) {
        if (
            _totalSupply == 0 || (lastTimeRewardApplicable() < lastUpdatedTime)
        ) {
            return 0;
        }
        return
            ...
            (
                ((rewardRate *
                    (lastTimeRewardApplicable() - lastUpdatedTime) *
                    1e18) / _totalSupply)
            ) /
            REWARD_RATE_MULTIPLIER;
    }
    ...
    function earned(address _account) public view returns (uint256) {
        return
            ((lpStakeholders[_account].userLpBalance *
                (rewardPerToken() -
                    lpStakeholders[_account].userRewardPerTokenPaid)) / 1e18) +
